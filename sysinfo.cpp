...
int main() {
    ...
    printf(" GB\n");
    std::cout << "Total Free RAM: "
              << (float)statex.ullAvailPhys / (1024 * 1024 * 1024);
    printf(" GB\n");
    if (!cpuid_present()) {  // check for CPUID presence // check for CPUID
                             // presence
        printf("CPU: Unknown\n");
    } else {
        ...
        if (cpuid_get_raw_data(&raw) < 0) {  // obtain the raw CPUID data
            ...
            printf("Error: %s\n", cpuid_error());  // cpuid_error() gives the
                                                   // last error description
            return -2;
        }
        if (cpu_identify(&raw, &data) <
            0) {  // identify the CPU, using the given raw data.
...
int main() {
    ...
    if (uname(&buffer) < 0) {
        ...
        printf("OS Name: %s\n",
               exec("cat /etc/*-release | egrep 'PRETTY_NAME' | cut -d = -f 2 "
                    "| tr -d '\"' | tac | tr '\n' ' '")
                   .c_str());
        printf("Bitness: %s\n", buffer.machine);
